...
sudo apt install pika-gameutils-meta ttf-mscorefonts-installer blender obs-studio kdenlive -y #install welcome apps
echo ""

echo "Installing Packages and Flatpaks"
sudo apt install unzip wget sshfs youtubedl-gui qbittorrent fonts-firacode thunderbird remmina handbrake-cli handbrake rpi-imager python-is-python3 python3-venv python3-pip wakeonlan gnome-clocks docker docker-compose npm nodejs -y # install packages i need
...
echo "*/15 * * * * env DISPLAY=:0 DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus /home/aki/.local/bin/sync-wallpaper" >>mycron #echo new cron into cron file (change wallpaper every 15th minute 0/15/30/45)
